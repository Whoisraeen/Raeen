...
for a given title.
=======
## Per-draw shader resolve: share the program, re-derive the values

Target: the measured `resolve_us=16` per draw (169 draws = 2.7 ms of an 18.6 ms
Dead Cells menu frame), with `resolve_hits=0 resolve_misses=512` and
`resolve_misses_same_program=506` / `miss_gs_sgpr_inside=506`.

The proposed fix (narrow `ResolvedShaderKey` so same-program binds hit) stays
rejected and is now rejected on a second, independent ground. `analysis.rs:2211`
latches `user_sgpr.value[..]` into its output, so a program-keyed hit would serve
a stale push constant — the `d21e727` defect class. Measured additionally: the
256-entry memo's linear key scan costs **0.028 us**, so the lookup was never the
expense. Narrowing the key would have traded correctness for nothing.

MEASURED cause instead: three deep copies on the per-bind path, none of which the
`resolve_hits`/`parse_hits` counters could show, because they all happen *inside*
a cache hit.

* `ShaderInstruction` is **696 bytes** (`mimg_nsa_addr: [ShaderOperand; 12]`
  alone is 432). `ParsedCodeCache` served every hit with `entry.code.clone()`, so
  a 100%-hot decode cache still deep-copied 89–348 KiB per stage per draw. Now an
  `Arc<ShaderCode>`; every consumer already took `&ShaderCode`.
* `TranslatedShader` is **14,228 bytes** and names all three stage ABIs while
  filling one, so each construction zeroed ~7.5 KiB of `Default` that is never
  read. `ResolvedShaders` is **10,516 bytes** and was copied three times per draw
  (construct, memo insert, memo get). Both now hold `Arc`s; the two stages a
  translation did not analyze point at shared empty singletons.
* `ShaderTranslateCache::translate_{vs,ps,cs}` each did
  `self.shader_map.clone()` — a full deep copy of every relocated
  `sceAgcCreateShader` record into the bind's analysis closure. Each
  `ShaderMappedData` owns **six** allocations (`direct_resource_offset`, four
  `sharp_resource_offset` vectors, `input_semantics`), so this was an
  **O(shaders ever created)** term on every stage of every draw. Now
  `Arc<ShaderMap>`, mutated by `Arc::make_mut` in `map_shader_metadata`.

SYNTHETIC before/after, `resolve_cost_per_bind_with_changing_user_sgprs` and
`resolve_cost_versus_created_shader_count` (release, min of 5, same machine, same
harness, per *stage* bind — a draw resolves two):

| scenario | before | after |
|---|---|---|
| 7-instruction program | 1.91 us | 1.65 us |
| 103-instruction program | 6.34 us | 2.04 us |
| 487-instruction program | 13.52 us | 3.13 us |
| + 64 created shaders mapped | 15.25 us | 1.27 us |
| + 512 created shaders mapped | 85.69 us | 1.27 us |

The map term was ~0.21 us per created shader per stage bind and is now flat. Not
an fps claim: the retail frame is unmeasured here and only the user can measure
it. The removed work is bounded above by the reported `resolve_us=16`/draw.

FAITHFULNESS: `fixture_shader_pair_spirv_is_byte_pinned` snapshots the exact
SPIR-V words of the fixture VS/PS pair through the real per-draw path
(`translate_vs` then `translate_ps` fed the vertex ABI, as `resolve_shaders`
composes them). The two `.snap` files were generated by running that test against
**pre-change `HEAD`** (721b721) and are unmodified — the post-change code
reproduces both modules byte for byte. Shape assertions:
`same_program_with_different_user_sgprs_shares_one_decoded_stream` (`Arc::ptr_eq`
+ `strong_count == 3`), `changed_program_bytes_do_not_reuse_the_retained_decode`,
`stage_abi_is_re_derived_per_bind_even_when_the_program_is_shared` (the ABI must
*not* be shared), `untouched_stage_abis_are_shared_singletons`,
`relocated_shader_metadata_is_shared_with_binds_not_copied_into_them`.

DELIBERATELY NOT DONE — the analysis *output* was not partitioned into
program-derived and draw-derived halves. Descriptor contents legitimately feed
generated SPIR-V here (texture type/format, V# stride, embedded constants all
appear in `append_bind_identity`), so a shadPS4-style program-only `Shader::Info`
is not available without redesigning codegen. With the copies gone the residual
is 1.3–3.1 us per stage bind, i.e. the analysis passes were never the 16 us. Also
measured and rejected: memoizing `WindowMem::dwords_at`'s speculative 4 KiB
out-of-window reads. At **0.097 us/call** even ~20 calls per draw is ~2 us, which
does not justify the append-only arena (and its `unsafe`) that a `&self`
