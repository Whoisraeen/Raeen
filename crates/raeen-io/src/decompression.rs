...
) -> Result<Vec<u8>, raeen_core::error::IoError> {
    match compression {
        ...
        CompressionType::Lz4 => {
            ...
            lz4_flex::decompress(data, uncompressed_size)
                .map_err(|e| raeen_core::error::IoError::DecompressionFailed(format!("LZ4: {}", e)))
        }
        CompressionType::Zstd => {
            debug!("Zstd decompress: {} bytes", data.len());
            zstd::stream::decode_all(data).map_err(|e| {
                raeen_core::error::IoError::DecompressionFailed(format!("Zstd: {}", e))
            })
        }
        CompressionType::Kraken => {
            warn!("Kraken decompression not available — Oodle is proprietary");
            Err(raeen_core::error::IoError::DecompressionFailed(
