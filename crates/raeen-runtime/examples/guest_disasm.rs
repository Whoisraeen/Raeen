...
fn main() {
    ...
    let decrypted =
        raeen_firmware::crypto::decrypt_self(&bytes, &raeen_firmware::crypto::NoKeysProvider)
            .expect("decrypted SELF passthrough");
    let module = raeen_firmware::parse_sprx(&decrypted.elf).expect("parse inner ELF");
    ...
    for value in args {
        if let Some(value) = value.strip_prefix("e:") {
            ...
            let tags = module
                .dynamic
                .as_deref()
                .map(raeen_firmware::dynlib::parse_sce_dynamic)
                .transpose()
                .expect("parse dynamic tags")
                .unwrap_or_default();
            let standard = raeen_firmware::dynlib::standard_dynamic_view(&module.segments, &tags);
            ...
            println!(
                "\n=== export {target:#018x} ===\nname={} value={:#x}",
                raeen_firmware::dynlib::nid_names::describe(target),
                export.value
            );
            continue;
        }
