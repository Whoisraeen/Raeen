...
use xps5x_firmware::{ModuleRegistry, UNRESOLVED_STUB_BASE, load_process};
...
/// subdirectory. Same pattern as `xps5x-kernel`'s filesystem tests.
fn app_dir(tag: &str, files: &[(&str, Vec<u8>)]) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("xps5x-transitive-{tag}-{}", std::process::id()));
...
fn load(dir: &std::path::Path, main: &[u8]) -> xps5x_firmware::LoadedProcess {
