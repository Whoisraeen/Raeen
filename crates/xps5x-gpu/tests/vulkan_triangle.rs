...
//! claim a passing GPU test that never ran, so setting `XPS5X_REQUIRE_VULKAN=1`
//! turns a skip into a hard failure. Use it to prove the test really executed:
//!
//! ```text
//! XPS5X_REQUIRE_VULKAN=1 cargo test -p xps5x-gpu --test vulkan_triangle -- --nocapture
//! ```
...
///
/// ```text
/// cargo test -p xps5x-gpu --test vulkan_triangle -- --ignored --nocapture
/// ```
///
/// Set `XPS5X_TRIANGLE_PPM` to choose the output path.
#[test]
...
fn dump_triangle_image() {
    ...
    let path = std::env::var("XPS5X_TRIANGLE_PPM").unwrap_or_else(|_| "triangle.ppm".to_owned());
