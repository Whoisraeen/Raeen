...
fn assert_center(pixels: &xps5x_gpu::vulkan::RenderedImage, expected: [u8; 4], label: &str) {
...
fn cached_texture_reuse_is_byte_identical_and_invalidates_on_content_change() {
    if std::env::var_os("XPS5X_NO_TEX_CACHE").is_some() {
        eprintln!("texture_cache: SKIP — XPS5X_NO_TEX_CACHE disables the cache under test");
        return;
    }
