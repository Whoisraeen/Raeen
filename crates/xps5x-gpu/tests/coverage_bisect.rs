...
//! messages — yet ZERO fragments reach any render target (`XPS5X_FORCE_CLEAR`
...
use xps5x_gpu::vulkan::shaders::{triangle_fragment_spirv, triangle_vertex_spirv};
use xps5x_gpu::vulkan::{VulkanBackend, validation_error_count};
...
/// `XPS5X_TITLE_VS=<path to .spv>` (dump one with `XPS5X_DUMP_SHADERS`).
///
...
fn title_translated_vs_covers_the_measured_quad() {
    let Ok(vs_path) = std::env::var("XPS5X_TITLE_VS") else {
        eprintln!("title_translated_vs_covers_the_measured_quad: SKIP — set XPS5X_TITLE_VS");
        return;
    };
    ...
    let raw = std::fs::read(&vs_path).expect("XPS5X_TITLE_VS file must be readable");
...
/// VS on the covering quad. Gated on `XPS5X_TITLE_SHADER_DIR` (a
/// `XPS5X_DUMP_SHADERS` directory). The VS is exonerated (variable 4 covers
...
fn title_translated_ps_sweep() {
    let Ok(dir) = std::env::var("XPS5X_TITLE_SHADER_DIR") else {
        eprintln!("title_translated_ps_sweep: SKIP — set XPS5X_TITLE_SHADER_DIR");
        return;
    };
    let vs_path = std::env::var("XPS5X_TITLE_VS").expect("also set XPS5X_TITLE_VS");
    ...
    // `XPS5X_TITLE_PS_ZERO=1` zeroes ALL of it instead — replicating what the
    ...
    let zero = std::env::var_os("XPS5X_TITLE_PS_ZERO").is_some();
