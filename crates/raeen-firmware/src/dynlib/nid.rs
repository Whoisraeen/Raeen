...
/// resolve to, precomputed from the names Raeen's HLE implements.
///
...
impl NidDatabase {
    ...
    /// Build the database from a live [`raeen_hle::HleRegistry`]: every
    ...
    /// (see [`raeen_hle::HleRegistry::register_nid`]).
    pub fn from_hle(hle: &raeen_hle::HleRegistry) -> Self {
...
mod nid_database_tests {
    ...
    /// The payoff test for broadening HLE coverage (see raeen-hle's
    ...
    fn explicit_equal_nids_resolve_by_provider_without_overwrite() {
        fn first(_ctx: &raeen_hle::HleContext, _args: &[u64]) -> u64 {
            1
        }
        fn second(_ctx: &raeen_hle::HleContext, _args: &[u64]) -> u64 {
            2
        }
        let hle = raeen_hle::HleRegistry::new();
