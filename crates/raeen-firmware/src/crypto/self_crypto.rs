...
/// [`raeen_loader::self_format::is_self_magic`], which owns the canonical list.
#[cfg(test)]
use raeen_loader::self_format::SELF_MAGIC;
...
fn parse_header(data: &[u8]) -> Result<SelfHeader, FirmwareError> {
    ...
    if !raeen_loader::self_format::is_self_magic(magic) {
