...
//! observed `logs/xps5x.log` reached **15 GB** across a night of Minecraft
//! runs. Two limits keep that bounded, and both matter:
//!
//! * **Rotation** ([`rotate`]) — each run starts a fresh file, keeping exactly
//!   one previous run as `xps5x.log.1`. A crash is investigated against *this*
...
pub const PREV_LOG_FILE_NAME: &str = "xps5x.log.1";
...
pub const MAX_LOG_BYTES_ENV: &str = "XPS5X_LOG_MAX_BYTES";
...
///
/// Any older `xps5x.log.1` is dropped — the retention policy is exactly two
...
/// `xps5x.log.1` rather than appending to it, and the run's own output is
...
mod tests {
    ...
    /// bounded file. Regression guard for the 15 GB `logs/xps5x.log`.
    #[test]
    ...
    fn rotate_moves_the_current_log_aside_and_keeps_one_previous() {
        let dir = std::env::temp_dir().join(format!("xps5x-rotate-test-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
