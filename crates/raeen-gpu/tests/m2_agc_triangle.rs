...
//!
//! ```text
//! RAEEN_REQUIRE_VULKAN=1 cargo test -p raeen-gpu --test m2_agc_triangle -- --nocapture
//! ```
...
use raeen_gpu::agc_exec::{AgcGpuSession, M2_DRAW_HEIGHT, M2_DRAW_WIDTH, build_m2_draw_dcb};
use raeen_gpu::vulkan::{CLEAR_COLOR, TRIANGLE_COLOR, unorm8, validation_error_count};
...
fn require_or_skip(err: &impl std::fmt::Display) -> bool {
    if std::env::var_os("RAEEN_REQUIRE_VULKAN").is_some() {
        panic!("RAEEN_REQUIRE_VULKAN is set but M2 draw failed: {err}");
    }
...
fn agc_pm4_draw_produces_visible_triangle() {
    ...
    let path = std::env::var("RAEEN_TRIANGLE_PPM").unwrap_or_else(|_| {
        std::env::temp_dir()
            .join("raeen_m2_triangle.ppm")
            .to_string_lossy()
            .into_owned()
    });
