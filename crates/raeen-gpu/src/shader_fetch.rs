...
pub struct ShaderTranslateCache {
    ...
    /// Metadata `sceAgcCreateShader` relocated, shared with the per-bind
    /// analysis closures rather than copied into them.
    ///
    /// Each [`ShaderMappedData`] entry needs SIX heap allocations to clone (its
    /// `direct_resource_offset` vector, its four `sharp_resource_offset`
    /// vectors, and `input_semantics`), so deep-copying the whole map into
    /// every stage's analysis closure cost `6 * shaders_created` allocations
    /// per stage per draw — work that grows with how many shaders the title has
    /// ever created and is thrown away at the end of the bind. The closures
    /// only ever call [`ShaderMap::find`], so an [`Arc`] serves them the same
    /// bytes; [`Self::map_shader_metadata`] mutates through `Arc::make_mut`,
    /// which finds the map uniquely owned once the closures have returned.
...
impl ShaderTranslateCache {
    ...
    pub fn translate_vs(
        ...
        let shader_map = Arc::clone(&self.shader_map);
        let vs = *vs;
        let sh_regs = *sh_regs;
