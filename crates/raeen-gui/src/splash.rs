...
//! [`raeen_gpu::AgcGpuSession::set_pending_splash`] *before* entering the
...
pub(crate) fn stage_boot_splash(eboot_path: &Path) {
    ...
    raeen_gpu::AgcGpuSession::set_pending_splash(splash);
}

fn load_pic0(eboot_path: &Path) -> Option<raeen_gpu::RenderedImage> {
...
mod tests {
    ...
    fn pic0_decodes_to_rgba_rendered_image() {
        let dir = std::env::temp_dir().join(format!("raeen-splash-test-{}", std::process::id()));
    ...
    fn missing_pic0_is_none() {
        let dir = std::env::temp_dir().join(format!("raeen-splash-missing-{}", std::process::id()));
