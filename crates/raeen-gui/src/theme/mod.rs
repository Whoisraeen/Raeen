//! Theme tokens for the Raeen Shell.
//!
...
//! original Raeen colors. SM2b adds real on-disk loading via [`loader`]:
...
pub fn install_fonts(ctx: &egui::Context, theme: &Theme) {
    ...
    if let Some(font_bytes) = &theme.assets.font {
        let name = "raeen-theme-font".to_string();
...
/// The default, original-asset Raeen theme (spec §6, §11 — zero Sony assets).
