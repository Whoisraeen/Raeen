...
//! optional `raeen-title.toml` alongside it, decide whether the folder is a
...
//! `raeen-title.toml`, return the parsed title/metadata/gradient, or `None`
...
/// The on-disk shape of `raeen-title.toml`. All fields except `title` are
...
/// The result of successfully parsing an `raeen-title.toml` string.
...
/// Parse an `raeen-title.toml` string. Returns `None` for malformed TOML or
...
/// raw text of `<folder>/raeen-title.toml`, if one exists and was
...
/// installed PS5 title ships. The user's `raeen-title.toml` (explicit
...
fn enrich_from_param_json(item: &mut LibraryItem, eboot: &Path, had_title_toml: bool) {
    ...
    let Ok(metadata) = raeen_loader::pkg::scan_game_directory(dir) else {
        return;
    };
...
/// game install and reading its optional `raeen-title.toml`. Non-directories,
...
pub fn scan_dir(root: &Path) -> Vec<LibraryItem> {
    ...
    for entry in entries.flatten() {
        ...
        let title_toml = std::fs::read_to_string(path.join("raeen-title.toml")).ok();
...
mod tests {
    ...
    fn scratch_dir(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join("raeen-gui-scan-tests").join(tag);
    ...
    fn scan_dir_finds_a_real_ps5_titles_nested_eboot() {
        let root = std::env::temp_dir().join(format!("raeen-scan-nested-{}", std::process::id()));
