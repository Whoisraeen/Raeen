...
/// highlight and a dark base. Used both for `raeen-title.toml`'s optional
...
/// when a game has no `raeen-title.toml`, or one without an explicit
...
/// from `raeen-title.toml`; the concept-style Home renders only the trophy
...
#[derive(Debug, Clone)]
pub struct GameMeta {
    /// Parsed from `raeen-title.toml` but not rendered by the concept-style
