...
pub(crate) fn sleep_interruptibly(
    ...
    while let Some((park, rest)) = sleep_step(remaining, SLEEP_TEARDOWN_SLICE) {
        ...
        remaining = rest;
        // A sleeping thread is a legitimate signal target, and this is a slice
        // boundary like any other blocking wait's, holding no lock — so a queued
        // Orbis exception is delivered here and the sleep then finishes its
        // remaining time (see [`crate::exception`]).
        //
        // The handler's own runtime is deliberately NOT deducted from
        // `remaining`: the guest asked to sleep for `requested`, and a signal
        // must never shorten that. Sleeping less than asked is a correctness
