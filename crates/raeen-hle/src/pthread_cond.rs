...
fn hle_cond_init(ctx: &HleContext, args: &[u64]) -> u64 {
    ...
    if cond == 0 {
        ...
        let state = std::sync::Arc::new(raeen_kernel::PthreadCond::default());
        state
            .monotonic
...
fn condition(ctx: &HleContext, cond: u64) -> Option<std::sync::Arc<raeen_kernel::PthreadCond>> {
...
fn note_wait_outcome(ctx: &HleContext, cond: u64, woken: bool) {
    if std::env::var_os("RAEEN_TRACE_COND").is_none() {
        return;
    }
...
mod tests {
    ...
    fn fixture() -> (raeen_kernel::OrbisKernel, TestMemory, TestAllocator) {
        (
