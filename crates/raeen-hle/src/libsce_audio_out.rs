...
//! so the ring buffer paces itself), repeat. Raeen does not yet play audio,
    ...
    if ptr != 0 {
        ...
        if byte_len > 0 && byte_len <= MAX_OUTPUT_BYTES {
            ...
            if ctx.mem.read(ptr, &mut buf) {
                ...
                raeen_audio::output::submit(freq, &stereo);
            }
        }
    }
...
mod tests {
    ...
    fn port_table_is_bounded_and_close_releases_capacity() {
        ...
        for _ in 0..raeen_kernel::OrbisKernel::MAX_AUDIO_OUT_PORTS {
