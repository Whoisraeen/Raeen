...
//! [`xps5x_core::VERSION`], the Settings → System screen offers to download
...
/// Hard cap on a downloaded update, far above any plausible xps5x.exe.
...
pub fn apply_staged(staged: &Path) -> Result<(), String> {
    ...
    let script_path = exe.with_file_name("xps5x-update.bat");
...
mod tests {
    ...
    fn parse_latest_release_picks_the_bare_exe_asset() {
        let json = r#"{
            "tag_name": "v0.2.0",
            "assets": [
                {"name": "xps5x-windows-x86_64.zip",
                 "browser_download_url": "https://github.com/r/releases/download/v0.2.0/xps5x-windows-x86_64.zip"},
                {"name": "xps5x-windows-x86_64.exe",
                 "browser_download_url": "https://github.com/r/releases/download/v0.2.0/xps5x-windows-x86_64.exe"}
            ]
        }"#;
        let info = parse_latest_release(json).unwrap();
        assert_eq!(info.tag, "v0.2.0");
        assert!(info.exe_url.ends_with("xps5x-windows-x86_64.exe"));
    }
    ...
    fn parse_latest_release_refuses_plain_http() {
        let json = r#"{"tag_name": "v0.2.0", "assets": [
            {"name": "xps5x-windows-x86_64.exe",
             "browser_download_url": "http://evil.example/xps5x-windows-x86_64.exe"}
        ]}"#;
    ...
    fn swap_script_waits_swaps_relaunches_and_self_deletes() {
        let exe = Path::new(r"C:\apps\xps5x\xps5x.exe");
    ...
    fn staging_path_sits_next_to_the_exe() {
        assert_eq!(
            staging_path(Path::new(r"C:\apps\xps5x\xps5x.exe")),
            PathBuf::from(r"C:\apps\xps5x\xps5x.exe.update")
        );
    }
    ...
    fn status_and_action_labels_track_state() {
        ...
        let staged = UpdaterState::Staged {
            tag: "v0.2.0".to_string(),
            staged: PathBuf::from(r"C:\x\xps5x.exe.update"),
        };
