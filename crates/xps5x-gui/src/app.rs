...
mod tests {
    ...
    fn scan_game_folders_reads_every_configured_folder() {
        let base = std::env::temp_dir().join(format!("xps5x-scan-{}", std::process::id()));
    ...
    fn scan_game_folders_dedupes_a_folder_listed_twice() {
        let base = std::env::temp_dir().join(format!("xps5x-scan-dup-{}", std::process::id()));
    ...
    fn scan_game_folders_skips_missing_folders() {
        let missing = PathBuf::from("this/folder/does/not/exist/xps5x");
