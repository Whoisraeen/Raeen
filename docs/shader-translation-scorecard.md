...
- Report: `artifacts/compat/phase0-single-translator-rerun-44460ff-dirty.json`
- Build identity: `44460ffc9e10+dirty` (release, `max-fps`)
- Protocol: eight titles, 180 seconds each, compared with
  `artifacts/compat/phase3-baseline-4089-dirty.json`
- Result: 8 unchanged, 0 improved, 0 regressed
- Canaries: GTA V 12,096 flips and Minecraft 14,857 flips; both had zero
  shader errors and zero GPU errors

The accepted run followed a rejected run of the same executable in which
Subnautica stopped at 648 flips after its pre-existing URI-parser guest fault
fed a mutex deadlock. The accepted rerun reached 4,805 flips. Both outcomes are
retained in the progress ledger; the passing run is not used to erase the
...
| Failure corpus and clustering | D | Shader Phase 1 has not started. There is no hash-deduplicated binary corpus, fan-out report, or offline replay command yet. |
...
Overall shader grade remains **C+ (6.3/10)**. Phase 0 improved architectural
trustworthiness, not instruction compatibility, so it does not justify raising
the overall grade.
...
1. **Green — Phase 0:** exactly one translator reachable from the commercial
   pipeline.
2. **Next — Phase 1:** `cargo xtask shader-corpus` capture, ranked report, and
   seconds-scale offline replay from the eight-title corpus.
3. **Phase 2:** Avatar under 100 unique failure clusters, Subnautica at zero,
   Minecraft/GTA V still zero shader and GPU errors.
