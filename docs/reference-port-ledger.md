...
| emulator/Graphics: libSceAgc DCB emitters (cursor + PM4) | `xps5x-hle` | `wip` | `67d3399`,`1745228`,`217d4f7`,`3a8d787`,`ee492b2`,`3653788` | `libsce_agc.rs` — port of SharpEmu Agc's Draw-Command-Buffer cursor model (`alloc_command_dwords`: CB struct cursor-up@0x10/down@0x18/reserved@0x30) + **13 emitters** across DCB/Cb/Acb (draws, index setup, instances, markers, dispatch, flip, resets, nop, wait-flip, SH-register-range copy), emitting PM4 in Agc's dialect (`0xC000_0000\|(len-2)<<16\|op<<8\|(reg&0x3F)<<2`). Verified vs SharpEmu's exact bytes + cursor advance. 11 tests. Remaining Agc: the other ~49 DCB/ACB/Cb emitters (mechanical, same model) + register-defaults tables — all land with the Vulkan submit path that consumes the buffer; `Ampr` = the compute/DMA sibling. |
