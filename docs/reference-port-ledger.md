...
| AudioOut (pacing stub, no playback yet) | Audio | `xps5x-hle` | `done` | (audioout batch) | libsce_audio_out.rs: Init/Open(handle+grain/freq)/Output(acks buffer, sleeps ~grain÷freq bounded, returns sample count)/Close/SetVolume — audio thread paces without hang or 100% spin (M3 "audio must not hang"). Real host playback (cpal) is the follow-up. 3 tests |
