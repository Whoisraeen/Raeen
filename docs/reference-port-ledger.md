...
| emulator/Libs (libSce* HLE surface) | `xps5x-hle` | `wip` | (many) | realized as xps5x-hle libSce* modules: libc, libkernel (mem/time/module/proc-time), Sysmodule, PlayGo, User/System Service, Pad, AudioOut, VideoOut, SaveData, CommonDialog/MsgDialog, AppContent, Np, Net/NetCtl, DiscMap, **Rtc**. The high-traffic boot/HUD libs are done, plus Rtc + Mouse/Ime/GameUpdate; remaining SharpEmu libSce modules not yet ported (each its own future row): Rtc `GetCurrent*` host-clock family, Voice/Ngs2/Ajm/AvPlayer (media), Share, SystemGesture, NpGameIntent, Ult, Apr, Socket. GraphicsDriver = the M2 pipeline row. |
...
| Mouse / Ime / GameUpdate (headless) | Libs | `xps5x-hle` | `done` | `3de1121` | new libsce_peripheral.rs: libSceMouse (Open/Close/Read → 0 entries, no mouse), libSceIme (Update/Keyboard* → no pending events, no on-screen-keyboard session), libSceGameUpdate (Initialize/Terminate OK, no store). Resolves NIDs real titles poll (Quake KEX calls `sceImeUpdate` each frame) that were unresolved → crash. SharpEmu-cross-checked. 1 test. |
| Rtc — calendar math + current tick | Libs | `xps5x-hle` | `done` | `726ab2c`,`bd9d79e` | new libsce_rtc.rs: port of SharpEmu RtcExports. 23 NIDs — GetTickResolution/IsLeapYear/GetDaysInMonth/GetDayOfWeek (Sakamoto, Sun=0), CheckValid (field-by-field `SceRtcDateTime` validation w/ Orbis error codes), TickAdd{Ticks/µs/Sec/Min/Hour/Day/Week} (signed µs tick arithmetic, over/underflow→error), CompareTick(-1/0/1), **GetCurrentTick + Network/RawNetwork/AdNetwork/DebugNetwork aliases** (host UTC as an Rtc tick). Tick = µs since 0001-01-01. 5 tests. Only `GetCurrentClock` (tick→datetime *struct*) remains = a follow-up. |
