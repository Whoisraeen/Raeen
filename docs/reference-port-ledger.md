...
| emulator/Libs (libSce* HLE surface) | `xps5x-hle` | `wip` | (many) | realized as xps5x-hle libSce* modules: libc, libkernel (mem/time/module/proc-time), Sysmodule, PlayGo, User/System Service, Pad, AudioOut, VideoOut, SaveData, CommonDialog/MsgDialog, AppContent, Np, Net/NetCtl, DiscMap, **Rtc**. The high-traffic boot/HUD libs are done, plus Rtc + Mouse/Ime/GameUpdate; remaining SharpEmu libSce modules not yet ported: **media codecs Ngs2/Ajm/AvPlayer** (real DSP/decode — *not* honestly stubbable; a silent no-op would be the "no-op instead of real execution" anti-pattern) and **Agc/Ampr/Fiber/Ult** (overlap the M2-GPU / M1-E scheduler infrastructure gates, not standalone-portable). i.e. every *standalone, honestly-portable* libSce is now done; what remains is codec-DSP or infrastructure-gated. GraphicsDriver = the M2 pipeline row. |
