...
| DualSense: live input routing (kernel snapshot → scePadReadState) | Input | `xps5x-kernel`/`hle` | `done` | (padlive batch) | kernel holds a settable 12-byte pad-state snapshot (OrbisKernel::set_pad_state/pad_state); scePadReadState reads it (neutral fallback when unset) — live host input now flows guest-ward. Remaining: Shell polling InputManager into set_pad_state each frame (UI wiring) + haptics/adaptive triggers |
