...
| emulator/Libs (libSce* HLE surface) | `xps5x-hle` | `wip` | (many) | realized as xps5x-hle libSce* modules: libc, libkernel (mem/time/module/proc-time), Sysmodule, PlayGo, User/System Service, Pad, AudioOut, VideoOut, SaveData, CommonDialog/MsgDialog, AppContent, Np, Net/NetCtl, DiscMap, **Rtc**. The high-traffic boot/HUD libs are done; remaining SharpEmu libSce modules not yet ported (each its own future row): Rtc `GetCurrent*` host-clock family, Ime, Rtc/Voice/Ngs2/Ajm/AvPlayer (media), Share, SystemGesture, Mouse, GameUpdate, NpGameIntent, Ult. GraphicsDriver = the M2 pipeline row. |
