...
| emulator/Libs (libSce* HLE surface) | `xps5x-hle` | `wip` | (many) | realized incrementally as xps5x-hle libSce* modules: libc, libkernel (mem/time/module/proc-time), Sysmodule, PlayGo, User/System Service, Pad, AudioOut, VideoOut, SaveData, CommonDialog/MsgDialog, AppContent. Remaining Kyty Libs: Net, DiscMap, GraphicsDriver (M2), Np/trophy |
...
| Np (PSN) manager — offline | Np | `xps5x-hle` | `done` | (np batch) | new libsce_np.rs: sceNpGetState reports SIGNED_OUT (SharpEmu value 1), reachability UNAVAILABLE, GetOnlineId="Player", callbacks accepted-but-never-fire — a title sees "offline", disables online features, and boots instead of hanging on a PSN check. Online Np (trophy sync, matchmaking) out of scope. 2 tests |
