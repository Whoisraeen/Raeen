...
| Rtc — calendar math (pure) | Libs | `xps5x-hle` | `done` | `726ab2c` | new libsce_rtc.rs: port of the host-time-free half of SharpEmu RtcExports. 18 NIDs — GetTickResolution/IsLeapYear/GetDaysInMonth/GetDayOfWeek (Sakamoto, Sun=0), CheckValid (field-by-field `SceRtcDateTime` validation w/ Orbis error codes), TickAdd{Ticks/µs/Sec/Min/Hour/Day/Week} (signed µs tick arithmetic, over/underflow→error), CompareTick(-1/0/1). Tick = µs since 0001-01-01. 4 tests. `GetCurrent*Tick` (host-clock) = follow-up row. |
