...
| Sysmodule load chain (system modules, all HLE) | Libs | `xps5x-hle` | `done` | (sysmodule batch) | sceSysmoduleLoadModule/LoadModuleInternalWithArg/Unload/IsLoaded all succeed — every SCE_SYSMODULE_* is HLE-registered, so "loading" resolves without bringing anything into memory. Panic-safe arg access + test |
