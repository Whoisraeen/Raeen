...
**Goal:** Build the `xps5x-firmware` crate that ingests PS5 firmware, decrypts SELF through a user-supplied key boundary, and loads + NID-links Sony's real userland modules against either HLE reimplementations or other real modules (LM0 → LM1).

**Architecture:** A new `xps5x-firmware` crate sits on top of `xps5x-loader` (ELF/SELF primitives) and `xps5x-core` (errors, types, the `KeyProvider` trait). It adds a PUP container parser, a `.sprx`/SCE-module parser, a `PT_SCE_DYNLIBDATA` decoder, a NID-based dynamic linker, and a `ModuleRegistry` that dispatches each import to an HLE function (from `xps5x-hle`) or an exported symbol of another loaded module. Decryption machinery is added to `xps5x-loader::self_format`, driven entirely by a `KeyProvider`; the default provider supplies no keys.
...
**New crate `crates/xps5x-firmware/`:**
...
- `crates/xps5x-core/src/error.rs` — add `FirmwareError`, wire into `XPS5XError`.
- `crates/xps5x-core/src/lib.rs` — add `pub mod crypto;`.
- `crates/xps5x-core/src/crypto.rs` — **new**: `KeyProvider`, `KeyRequest`, `SegmentKey`, `NoKeysProvider`.
- `crates/xps5x-loader/Cargo.toml` — add `aes`, `cbc`.
- `crates/xps5x-loader/src/self_format.rs` — add `parse_self_with_provider`, AES-CBC decrypt; route `parse_self` through it.
- `crates/xps5x-hle/src/lib.rs` — add `HleRegistry::entries()` enumerator.
- `crates/xps5x-hle/Cargo.toml` — (unchanged; firmware depends on hle, not vice-versa).

---

## Task 1: Scaffold `xps5x-firmware` crate + `FirmwareError`

**Files:**
- Modify: `Cargo.toml` (root) — members + workspace deps
- Modify: `crates/xps5x-core/src/error.rs`
- Create: `crates/xps5x-firmware/Cargo.toml`
- Create: `crates/xps5x-firmware/src/lib.rs`

**Interfaces:**
- Produces: crate `xps5x-firmware` (buildable); `xps5x_core::error::FirmwareError` enum with variants `InvalidPupMagic(u32)`, `PupEntryOutOfBounds { index: usize }`, `MissingKey { key_id: u64 }`, `UnsupportedRelocation(u32)`, `MalformedDynlibData(String)`, `Loader(#[from] LoaderError)`; `XPS5XError::Firmware(#[from] FirmwareError)`.
...
base64 = "0.22"
```
In the same section, after `xps5x-gui = { path = "crates/xps5x-gui" }`, add:
```toml
xps5x-firmware = { path = "crates/xps5x-firmware" }
```
In `[workspace] members`, after `"crates/xps5x-gui",` add:
```toml
    "crates/xps5x-firmware",
```

- [ ] **Step 2: Add `FirmwareError` to `crates/xps5x-core/src/error.rs`**

Add a new variant to `XPS5XError` (after the `Io` variant, before `Config`):
```rust
...
- [ ] **Step 3: Create `crates/xps5x-firmware/Cargo.toml`**
...
- [ ] **Step 4: Create `crates/xps5x-firmware/src/lib.rs`**

```rust
//! # XPS5X Firmware
//!
//! The "firmware spine": ingests PS5 firmware packages (PUP), decrypts SELF
//! modules through a user-supplied [`xps5x_core::crypto::KeyProvider`], parses
//! Sony's SCE dynamic modules, and links their imports by NID against either
//! HLE reimplementations (`xps5x-hle`) or exports of other loaded modules.
//!
//! ## Clean-room boundary
//!
...
Run: `cargo test -p xps5x-firmware -p xps5x-core`
Expected: compiles; `crate_builds` passes; `xps5x-core` still passes.

- [ ] **Step 6: Commit**

```bash
git add Cargo.toml Cargo.lock crates/xps5x-core/src/error.rs crates/xps5x-firmware/
git commit -m "$(cat <<'EOF'
feat(firmware): scaffold xps5x-firmware crate and FirmwareError
...
- Modify: `crates/xps5x-firmware/src/lib.rs` (add `pub mod pup;`)

**Interfaces:**
- Consumes: `xps5x_core::error::FirmwareError`.
- Produces:
...
//! separate, explicit, key-gated step (see `xps5x_loader::self_format`).
...
impl Firmware {
    ...
    pub fn open(path: impl AsRef<Path>) -> Result<Firmware, FirmwareError> {
        let bytes = std::fs::read(path).map_err(|e| {
            FirmwareError::Loader(xps5x_core::error::LoaderError::Io(e))
        })?;
...
In `crates/xps5x-firmware/src/lib.rs`, add after the `VERSION` const:
```rust
pub mod pup;
```
...
**Files:**
- Create: `crates/xps5x-firmware/src/bin/firmware-info.rs`

**Interfaces:**
- Consumes: `xps5x_firmware::pup::Firmware`.
- Produces: binary `firmware-info` runnable via `cargo run -p xps5x-firmware --bin firmware-info -- <path>`.

- [ ] **Step 1: Write an integration test for the library path the binary uses**

Create `crates/xps5x-firmware/tests/firmware_info.rs`:
```rust
//! Exercises the same `Firmware::open` path the `firmware-info` binary uses.

use std::io::Write;
use xps5x_firmware::pup::{Firmware, PUP_MAGIC};

#[test]
fn opens_a_synthetic_pup_file_from_disk() {
    let dir = std::env::temp_dir();
    let path = dir.join("xps5x_test_synthetic.pup");
...
Run: `cargo test -p xps5x-firmware --test firmware_info`
...
use xps5x_firmware::pup::Firmware;
...
The binary uses `tracing_subscriber`. Add it to `crates/xps5x-firmware/Cargo.toml` under `[dependencies]`:
```toml
...
Run: `cargo test -p xps5x-firmware --test firmware_info`
Then: `cargo build -p xps5x-firmware --bin firmware-info`
Expected: both succeed.

- [ ] **Step 6: Manual LM0 acceptance against the real firmware**

Run: `cargo run -p xps5x-firmware --bin firmware-info -- "PS5 Firmware/PS5UPDATE.PUP"`
...
```bash
git add crates/xps5x-firmware/
git commit -m "$(printf 'feat(firmware): add firmware-info diagnostic binary (LM0)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>')"
```

---

## Task 4: `KeyProvider` trait + `NoKeysProvider` in `xps5x-core`

**Files:**
- Create: `crates/xps5x-core/src/crypto.rs`
- Modify: `crates/xps5x-core/src/lib.rs` (add `pub mod crypto;`)
...
//!
//! XPS5X never contains keys. A [`KeyProvider`] is supplied by the user from
...
///
/// Implementors are user-provided. XPS5X ships only [`NoKeysProvider`].
...
In `crates/xps5x-core/src/lib.rs`, add to the module list (after `pub mod config;`):
```rust
pub mod crypto;
```
...
Run: `cargo test -p xps5x-core crypto`
Expected: `no_keys_provider_returns_none` and `key_provider_is_object_safe` pass.

- [ ] **Step 4: Commit**

```bash
git add crates/xps5x-core/src/crypto.rs crates/xps5x-core/src/lib.rs
git commit -m "$(printf 'feat(core): add KeyProvider decryption boundary\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>')"
```

---

## Task 5: SELF decryption seam in `xps5x-loader`

**Files:**
- Modify: `crates/xps5x-loader/Cargo.toml`
- Modify: `crates/xps5x-loader/src/self_format.rs`

**Interfaces:**
- Consumes: `xps5x_core::crypto::{KeyProvider, KeyRequest, SegmentKey, NoKeysProvider}`.
- Produces:
...
In `crates/xps5x-loader/Cargo.toml` `[dependencies]`, add:
```toml
aes = { workspace = true }
cbc = { workspace = true }
```
...
In `crates/xps5x-loader/src/self_format.rs`, replace the existing `#[cfg(test)] mod tests { ... }` block with this expanded version:
```rust
#[cfg(test)]
mod tests {
    use super::*;
    use aes::cipher::{BlockEncryptMut, KeyIvInit};
    use xps5x_core::crypto::{KeyProvider, KeyRequest, SegmentKey};
...
use xps5x_core::crypto::{KeyProvider, KeyRequest, NoKeysProvider};
...
(d) Add the loader's dep on `xps5x-core::crypto` — no manifest change needed (`xps5x-core` is already a dependency); the `use` in (a) is sufficient.
...
```bash
git add crates/xps5x-loader/Cargo.toml crates/xps5x-loader/src/self_format.rs Cargo.lock
...
- Modify: `crates/xps5x-firmware/src/lib.rs` (add `pub mod sprx;`)
...
In `crates/xps5x-firmware/src/lib.rs`, add:
```rust
pub mod sprx;
```
...
Run: `cargo test -p xps5x-firmware sprx`
Expected: `extracts_segments_and_dynlibdata` and `rejects_non_elf` pass.

- [ ] **Step 4: Commit**

```bash
git add crates/xps5x-firmware/src/sprx.rs crates/xps5x-firmware/src/lib.rs
...
**Files:**
- Create: `crates/xps5x-firmware/src/dynlib/mod.rs` (module root; grown in Task 8)
...
/// Maps NIDs back to the symbol names XPS5X knows.
...
```bash
git add crates/xps5x-firmware/src/dynlib/ crates/xps5x-firmware/src/lib.rs
...
Replace the contents of `crates/xps5x-firmware/src/dynlib/mod.rs` with:
```rust
//! Sony dynamic-linking data: NID hashing, PT_SCE_DYNLIBDATA decoding, and
//! the NID-based linker.
...
Run: `cargo test -p xps5x-firmware dynlib::tests`
Expected: `parses_imports_exports_and_relocs` and `rejects_unterminated_tag_array` pass.

- [ ] **Step 3: Commit**

```bash
git add crates/xps5x-firmware/src/dynlib/mod.rs
...
**Files:**
- Modify: `crates/xps5x-hle/src/lib.rs` (add `HleRegistry::entries()`)
- Create: `crates/xps5x-firmware/src/registry.rs`
- Create: `crates/xps5x-firmware/src/dynlib/linker.rs`
- Modify: `crates/xps5x-firmware/src/dynlib/mod.rs` (add `pub mod linker;`)
- Modify: `crates/xps5x-firmware/src/lib.rs` (add `pub mod registry;`)

**Interfaces:**
- Consumes: `xps5x_hle::{HleRegistry, HleFunction}`, `crate::dynlib::{DynlibInfo, ImportEntry, Relocation, nid::NidDatabase}`, `crate::sprx::Module`.
- Produces:
  - `xps5x_hle::HleRegistry::entries(&self) -> Vec<(String, String, HleFunction)>` → `(library, function, func)`.
...
In `crates/xps5x-hle/src/lib.rs`, add this method inside `impl HleRegistry` (after `is_implemented`):
```rust
...
use tracing::debug;
use xps5x_hle::{HleFunction, HleRegistry};
...
pub struct ModuleRegistry {
    /// NID → HLE implementation (bridged from `xps5x-hle` by hashing names).
...
In `crates/xps5x-firmware/src/dynlib/mod.rs`, add after `pub mod nid;`:
```rust
pub mod linker;
```
In `crates/xps5x-firmware/src/lib.rs`, add:
...
Run: `cargo test -p xps5x-firmware -p xps5x-hle`
Expected: registry (3) + linker (2) tests pass; `xps5x-hle` still green.

- [ ] **Step 6: Commit**

```bash
git add crates/xps5x-hle/src/lib.rs crates/xps5x-firmware/src/registry.rs crates/xps5x-firmware/src/dynlib/
...
**Files:**
- Create: `crates/xps5x-firmware/src/loader.rs`
- Modify: `crates/xps5x-firmware/src/lib.rs` (add `pub mod loader;` + re-exports)
- Create: `crates/xps5x-firmware/tests/lm1_end_to_end.rs`
...
use tracing::info;
use xps5x_core::crypto::KeyProvider;
use xps5x_core::error::FirmwareError;
...
/// SELF outer magic (mirrors `xps5x_loader::self_format`).
...
/// Mirrors the field layout used by `xps5x_loader::self_format`.
fn decrypt_self_inner(
    data: &[u8],
    provider: &dyn KeyProvider,
) -> Result<Vec<u8>, FirmwareError> {
    use xps5x_core::crypto::KeyRequest;
...
`loader.rs` uses `aes`/`cbc`. In `crates/xps5x-firmware/Cargo.toml` `[dependencies]`, add:
```toml
aes = { workspace = true }
cbc = { workspace = true }
```
...
use xps5x_core::crypto::NoKeysProvider;
use xps5x_firmware::dynlib::nid::nid_of;
use xps5x_firmware::{ModuleLoader, ModuleRegistry};
use xps5x_hle::HleRegistry;
...
Run: `cargo test -p xps5x-firmware --test lm1_end_to_end`
...
Run: `cargo test --workspace`
Then: `cargo clippy -p xps5x-firmware -- -D warnings`
Then: `cargo fmt -p xps5x-firmware`
Expected: tests pass; no clippy warnings; formatting clean.

- [ ] **Step 7: Commit**

```bash
git add crates/xps5x-firmware/ Cargo.lock
...
- §3.3.2 `KeyProvider` seam + SELF decryption → Tasks 4, 5. ✅ (trait relocated to `xps5x-core` to avoid a `loader ↔ firmware` cycle; noted in File Structure.)
...
- `HleFunction = fn(&[u64]) -> u64` — from existing `xps5x-hle`; used unchanged in Tasks 9, 10. ✅
