...
**Architecture:** The runtime reserves one fixed-base host region (`GUEST_ARENA_BASE = 0x0000_1000_0000_0000`, 4 GiB) and identity-maps the guest onto it (guest addr == host addr). The image maps at the base; heap/stack/mmap are fixed sub-regions. A new `GuestAllocator` trait (in `xps5x-hle`, alongside `GuestMemory`) is implemented by the runtime's `GuestArena` and threaded into `HleContext` so `malloc`/`mmap` HLE functions allocate real guest memory. The kernel `VirtualMemoryManager` becomes region *metadata* over the arena.

**Tech Stack:** Rust 2024, `windows-sys` (`VirtualAlloc`/`VirtualFree`, already a dep), existing VEH runtime.

**Spec:** `docs/superpowers/specs/2026-07-13-xps5x-guest-address-space-design.md` (read §2–§6 for the model; this plan implements milestones RT2a + RT2b).
...
- `xps5x-runtime` stays `#![forbid(unsafe_op_in_unsafe_fn)]`; **every** `unsafe` block carries a `SAFETY:` comment; clippy clean across `--workspace --all-targets`.
...
**Files:**
- Modify: `crates/xps5x-hle/src/lib.rs` (add `GuestAllocator` trait; add `alloc` field to `HleContext`; thread through `HleRegistry::call`; add a `TestAllocator` double in tests)
- Modify: `crates/xps5x-runtime/src/dispatch.rs` (`ActiveContext.alloc: *const dyn GuestAllocator`; `run` param; `veh_callback` builds `HleContext { kernel, mem, alloc }`)
- Modify: `crates/xps5x-runtime/src/lib.rs` (temporary: pass a stub allocator so the workspace compiles and existing tests pass — replaced in Task 3)
- Modify: `crates/xps5x-hle/src/{libkernel,libc}.rs` etc. only if `HleContext` construction sites need it (they take `&HleContext`, so no change beyond the registry)
- Test: `crates/xps5x-hle/src/lib.rs` tests updated to pass a `TestAllocator`
...
**Steps:**
- [ ] Add `GuestAllocator` trait and the `alloc: &'a dyn GuestAllocator` field to `HleContext` in `xps5x-hle`. Update every `HleContext { .. }` construction and `HleRegistry::call` usage. In hle unit tests, add a minimal `TestAllocator` (e.g. a bump over a `Cell<u64>`, `free`/`munmap` no-ops) and pass it.
...
- [ ] Run `cargo test -p xps5x-hle -p xps5x-runtime` and `cargo build --workspace`. Expected: all existing tests pass (behavior unchanged — nothing calls `alloc` yet).
...
**Files:**
- Create: `crates/xps5x-runtime/src/arena.rs`
- Modify: `crates/xps5x-runtime/src/lib.rs` (add `mod arena;`, export `pub const GUEST_ARENA_BASE`)
- Test: unit tests in `arena.rs`

**Interfaces:**
- Produces: `pub const GUEST_ARENA_BASE: u64 = 0x0000_1000_0000_0000;` and `pub(crate) struct GuestArena` with `fn new(image: &[u8]) -> Result<GuestArena, RuntimeError>` (reserves the region, commits sub-regions, copies `image` to the image region), `fn entry_ptr(&self, entry_offset: u64) -> Result<*const u8, RuntimeError>`, and `impl xps5x_hle::GuestMemory` + `impl xps5x_hle::GuestAllocator` for it.
...
- [ ] `cargo test -p xps5x-runtime`, `cargo clippy -p xps5x-runtime --all-targets`. Expected: pass, clean.
...
**Files:**
- Modify: `crates/xps5x-runtime/src/lib.rs` (`execute_linked` builds a `GuestArena`, maps image at base, passes `&arena` as both `mem` and `alloc`; drop the `NullAllocator` stub and `MappedImage` usage)
- Modify: `crates/xps5x-runtime/src/mem.rs` (retire `MappedImage`, or keep only an image-copy helper if `arena.rs` reuses it — no dead code)
- Modify: `crates/xps5x-gui/src/launcher.rs` (`DEFAULT_LOAD_BASE` → `xps5x_runtime::GUEST_ARENA_BASE`)
- Modify: `crates/xps5x-runtime/tests/execute.rs` (existing tests: link synthetic modules at `GUEST_ARENA_BASE`; guest-memory addresses in the `memcpy` test become `GUEST_ARENA_BASE + offset`)
...
- [ ] `cargo test -p xps5x-runtime -p xps5x-gui`, `cargo clippy --workspace --all-targets`. Expected: green, clean.
...
**Files:**
- Modify: `crates/xps5x-hle/src/libc.rs` (`malloc`/`calloc`/`realloc`/`free`/`memalign`/`posix_memalign` → `ctx.alloc` + `ctx.mem`; remove the `FAKE_HEAP_ADDR` sentinel)
- Test: `crates/xps5x-hle/src/libc.rs` unit tests (with `TestAllocator` + `TestMemory`); `crates/xps5x-runtime/tests/execute.rs` (end-to-end)
...
- [ ] `cargo test -p xps5x-hle -p xps5x-runtime`, `cargo clippy --workspace --all-targets`. Expected: green, clean.
...
**Files:**
- Modify: `crates/xps5x-kernel/src/memory/mod.rs` (add `record_mapping(&self, addr, size, prot)` inserting a `MemoryRegion` with no `Vec` backing; `remove_mapping(&self, addr)`)
- Modify: `crates/xps5x-hle/src/libkernel.rs` (`sceKernelMapFlexibleMemory`/`sceKernelAllocateDirectMemory`/`mmap` → `ctx.alloc.mmap` + `ctx.kernel.memory.record_mapping` + write out-param via `ctx.mem`; `munmap` → `ctx.alloc.munmap` + `remove_mapping`)
- Test: `crates/xps5x-kernel/src/memory/mod.rs` (record/remove metadata), `crates/xps5x-runtime/tests/execute.rs` (end-to-end mmap)
