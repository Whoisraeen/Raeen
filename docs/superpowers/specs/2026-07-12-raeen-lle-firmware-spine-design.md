# Raeen — LLE Firmware Spine Design

**Date:** 2026-07-12
**Status:** Approved (design), pending implementation plan
**Scope of this spec:** The "firmware spine" — the subsystem that ingests PS5 firmware, decrypts it through a user-supplied key boundary, loads Sony's real userland modules, and links them by NID against either real modules (LLE) or Raeen's reimplementations (HLE). Target milestones **LM0 → LM1** (see roadmap).
...
- **Keep** the HLE Orbis kernel (`raeen-kernel`) — syscalls, memory, threading, VFS are still reimplemented.
- **Add** the ability to load and execute Sony's **real userland modules** (`.sprx`) extracted from the firmware.
- **Per-module choice:** each imported symbol resolves to either Sony's real code (LLE) or Raeen's reimplementation (HLE), toggled by config.
...
1. **Raeen ships no keys, no firmware, and no key-extraction/circumvention tooling.** The repository never contains Sony keys or firmware blobs. (`PS5 Firmware/` is already `.gitignore`d.)
...
The `KeyProvider` interface is the hard stop. Raeen implements everything on the *emulator* side of that seam and nothing on the *attack* side of it.

---

## 3. Architecture

### 3.1 New crate: `raeen-firmware`

The spine is distinct from plain binary parsing and gets its own crate. It depends on `raeen-loader` (ELF/SELF primitives) and `raeen-core` (errors, types), and is consumed by `raeen-hle` (module registry) and, later, the runtime/GUI.
...
graph TD
    ...
    R -->|"HLE"| HLE["raeen-hle<br/>HleRegistry (name-keyed)"]
...
- Drives the `raeen --firmware-info <PUP>` diagnostic (LM0 acceptance).
...
- Parse Sony modules (`ET_SCE_DYNLIB` = `0xFE18`, and dynamic execs `ET_SCE_DYNEXEC` = `0xFE10`). The existing ELF parser already *recognizes* these types and the SCE program-header types but only logs them ([elf.rs:74-85](../../../crates/raeen-loader/src/elf.rs), [elf.rs:122-133](../../../crates/raeen-loader/src/elf.rs)).
...
- **`nid.rs`:**
  ...
  - Build the resolution database: for every symbol Raeen implements in HLE (known by name), precompute its NID so imports-by-NID resolve to HLE functions with no per-symbol hand-mapping.
...
- `ModuleRegistry` maps `(module_name_or_nid, symbol_nid)` → `Resolver`, where `Resolver` is one of:
  - `Hle(HleFunction)` — bridged to the existing name-keyed `HleRegistry` in `raeen-hle` via the NID→name database.
...
- Bridges to `raeen-hle::HleRegistry` (see [lib.rs:41](../../../crates/raeen-hle/src/lib.rs)) — the registry does not duplicate HLE implementations, it routes to them.

### 3.4 Error handling

- A dedicated `FirmwareError` enum now exists in `raeen-core::error` (added alongside this design) with: `InvalidPupMagic(u32)`, `PupEntryOutOfBounds { index }`, `MissingKey { key_id }`, `UnsupportedRelocation(u32)`, `MalformedDynlibData(String)`, and `Loader(#[from] LoaderError)`. It is wired into the top-level `RaeenError` as `Firmware(#[from] FirmwareError)`.
...
|:---|:---|:---|
| **LM0** | `raeen --firmware-info <PUP>` enumerates firmware contents; `KeyProvider` seam wired; default no-keys path is clean | No |
...
1. **Scaffold `raeen-firmware` crate**; add to workspace members; wire `raeen-core` + `raeen-loader` deps.
...
7. **`registry.rs` + `dynlib/linker.rs`** — `ModuleRegistry` with HLE/LLE dispatch, bridged to `raeen-hle::HleRegistry`; relocation application.
...
### Manual
- **LM0:** run `raeen --firmware-info "PS5 Firmware/PS5UPDATE.PUP"`; verify it enumerates entries and reports encrypted payloads as such without crashing and without attempting decryption.
