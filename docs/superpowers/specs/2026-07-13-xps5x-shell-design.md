# XPS5X Shell — Design Spec

**Status:** Approved direction (visual reference locked via interactive mockup)
**Date:** 2026-07-13
**Crate:** `xps5x-gui`
**Companion doc:** `docs/superpowers/specs/2026-07-12-xps5x-lle-firmware-spine-design.md` (the engine)

---

## 1. Goal

XPS5X launches into a **full-screen, PS5-style console experience** — its own dashboard ("the Shell") — that boots, lists the user's games, and hands a chosen title to the emulation engine to run with enhancements (unlocked framerate, higher resolution). To the person on the couch, **the app is a console**. The Shell is XPS5X's own software; it is not Sony's `SceShellUI`.
...
The Shell recreates the **layout, motion language, and interaction model** of the PS5 home — none of which are copyrightable. It does **not** ship Sony's copyrighted assets: the SST font, Sony's icon artwork, sound effects, dynamic background art, logos/wordmarks, or any game box-art. Those are replaced with original XPS5X assets.

**Authenticity path — theming.** The Shell is asset-driven through a theme layer (§6). XPS5X ships a **default original theme**. A user may install a **local theme** built from assets they extracted from firmware/hardware they own. That local theme never ships with XPS5X and is never committed to the repo. This keeps the distributable clean while allowing a personal build to look exactly like the real thing.
...
The reference is the interactive mockup (`scratchpad/xps5x-shell.html`). Screens, in build priority:
...
```
themes/
  default/            # ships with XPS5X — original assets only
