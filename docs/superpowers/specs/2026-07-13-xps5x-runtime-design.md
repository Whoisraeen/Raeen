# XPS5X Execution Runtime — Design Spec

**Date:** 2026-07-13
**Status:** Design (pending plan)
**Scope:** The subsystem that takes a **linked module** (`xps5x_firmware::LinkedModule`) and actually **executes** its code, dispatching each HLE import call to the `HleRegistry`. This is the step from "a homebrew `.sprx` *links*" (LM1) to "its code *runs*."
**Builds on:** LM1 firmware spine (`load_module` → `LinkedModule { image, base, hle_trampolines, unresolved }`), `xps5x-hle::HleRegistry`, and the SM3 `FirmwareLauncher` seam in the shell.

---

## 1. Goal & non-goals

**Goal (the runtime track):** run PS5 userland module code natively and service its library calls through XPS5X's HLE. The PS5 CPU is x86-64 (Zen 2) and the host (Windows) is x86-64, so module code executes **natively** — no CPU interpreter or JIT (per `implementation_plan.md` and the firmware-spine design). The runtime's job is the *environment*: guest memory, the ABI boundary, and trapping the HLE trampoline addresses the linker planted so a guest call to an imported symbol becomes a call into a Rust HLE function.
...
- The guest function XPS5X calls at entry is declared `extern "sysv64"` so Rust passes RT0's test arguments in the convention the guest expects.

## 4. Components (`xps5x-runtime`, new crate)

```
crates/xps5x-runtime/
└── src/
...
pub fn execute_linked(
    module: &xps5x_firmware::LinkedModule,
...
- Rust edition 2024, rust-version ≥ 1.85, GPL-2.0-only. New crate `xps5x-runtime` added to the workspace; depends on `xps5x-firmware`, `xps5x-hle`, `xps5x-core`, and `windows`/`windows-sys` for VEH + `VirtualAlloc` (a new dependency — the first OS-API crate; scoped to this crate).
