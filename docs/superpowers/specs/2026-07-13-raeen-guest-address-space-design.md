# Raeen Guest Address Space — Design Spec
...
**Builds on:** the RT0/RT1 runtime (`execute_linked`, VEH dispatch, `MappedImage`), the HLE dispatch context (`HleContext { kernel, mem }`, `GuestMemory`), and `raeen_kernel::VirtualMemoryManager`.
...
New allocation seam in `raeen-hle` (parallel to the existing `GuestMemory` trait, so `raeen-hle` needs no dependency on `raeen-runtime`):
