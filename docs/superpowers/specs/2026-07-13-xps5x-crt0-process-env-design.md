# XPS5X crt0 / Process Environment (Wall #1) — Design Spec
...
A new `crates/xps5x-runtime/src/process.rs` builds the initial stack in the arena's stack region (top-down), returning the `rsp` value `_start` must see:
