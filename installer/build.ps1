<#
    build.ps1 - build the Raeen release, then package it two ways:

      1. dist\Raeen-<version>-Setup.exe        (Inno Setup installer)
      2. dist\Raeen-<version>-portable-win64.zip (portable, extract-and-run)
    ...
    Usage (from anywhere):
      ...
      installer\build.ps1 -SkipBuild            # reuse target\release\raeen.exe
...
[CmdletBinding()]
param(
    [switch]$SkipBuild,      # reuse an existing target\release\raeen.exe
...
$ReleaseExe   = Join-Path $RepoRoot 'target\release\raeen.exe'
...
Write-Step "Raeen packaging - version $Version"
...
if (-not $SkipPortable) {
    ...
    $portableConfig = @"
# Raeen portable configuration. Paths are relative to this folder.
...
"@ -Encoding UTF8
    ...
    $zip = Join-Path $DistDir "Raeen-$Version-portable-win64.zip"
...
Get-ChildItem $DistDir -File | Where-Object { $_.Name -like "Raeen-$Version-*" } |
